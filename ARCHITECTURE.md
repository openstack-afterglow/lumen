...
| --- | --- | --- | --- | --- |
| Native conversation/temp durable run admission | implemented | 1283 core contract, 40 real MariaDB/Redis integration and 9 container system tests passed (2026-09-24); cloud-backed plan/code E2E pending | persistent native plan/code는 v2 checkpointer, 명시적 agent budget, operator quota를 요구하며 code는 enabled sandbox pool도 요구한다. API/SSE가 실행 수명을 소유하지 않는다 | `lumen/api/completions.py`, `lumen/services/chat_admission.py`, `lumen/services/durable_runs/admission.py` |
...
| Provider/model routing, credentials, billing | implemented | focused billing/admin-key/migration tests passed (2026-09-13); billing/admin/routing/migration/cache-pricing tests re-run after the token breakdown (96 passed, 2026-09-23) | 공개 `api_model_name`/`api_provider`와 내부 LiteLLM route key를 분리한다. Bulk 관리자 projection은 모든 provider의 Lumen 귀속 일·주·월·누적 ledger usage와 고정 공식 console 링크를 반환한다. OpenRouter/DeepSeek는 inference key 기반 live 잔액을, direct OpenAI/Anthropic은 별도 암호화 administrator key 기반 현재 UTC 일·주·월 조직 cost/usage를 제공한다. Gemini는 console-only이고 Perplexity Computer Analytics는 API Platform billing과 범위가 달라 호출하지 않는다. Local ledger와 Anthropic 조직 report는 추가로 `uncached_input`/`cache_read`/`cache_creation_5m`/`cache_creation_1h`/`output` 범주별 token breakdown을 반환한다(OpenAI report는 분할이 없어 `null`). Provider/report 실패는 fail-soft로 격리한다. | `lumen/services/providers/repository.py`, `lumen/services/providers/routing.py`, `lumen/services/providers/credentials.py`, `lumen/services/providers/billing.py`, `lumen/api/models.py`, `lumen/migrations/011_provider_billing_admin_key.sql` |
...
| Native managed tools, skills, MCP | implemented (default plugin providers) | full core contract suite 1283 passed plus 88 standalone plugin conformance tests passed and 1 skipped, 2026-09-24 | 다섯 관리자 설치 wheel은 실행 프로세스 안에서 신뢰된 코드다. 사용자 업로드 Python의 동적 설치는 제공하지 않는다; 실제 외부 MCP/플러그인 권한 서비스는 live로 확인하지 않았다 | `lumen/plugins/`, `plugins/`, `lumen/services/tool_runtime/`, `docs/agent-platform.md` |
...
| Provider-native server-side compaction | implemented | focused compaction tests passed (78 passed, 2026-09-22); synthetic provider payloads only — no live Anthropic or OpenAI compaction observed | Anthropic on the chat path, Anthropic and OpenAI Responses on the stateless proxies, because those are the transports whose pinned LiteLLM surface forwards `context_management`. Trigger is absolute with a per-protocol floor (50,000 Anthropic, 1,000 Responses) and is left unarmed when the window cannot express the ratio. Compaction blocks round-trip within a run. A proxy caller that sent its own `context_management` is never overridden. Billing of the compaction iteration, including its cache read/creation counters, is summed per iteration the same way on the chat path (LiteLLM `calculate_usage`) and the `/v1/messages` passthrough (`usage_iteration_breakdown`); see the prompt-cache row for how those counters are priced. | `lumen/services/native_compaction.py`, `lumen/services/graph.py`, `lumen/services/durable_runs/execution.py`, `lumen/services/completion_api.py` |
| Provider prompt caching, usage, pricing and ledger | implemented; direct OpenAI/Anthropic live acceptance pending | Focused cache/accounting and installed LiteLLM Anthropic wire-transform tests passed; contract 1270 passed, SDK 125 passed, real MariaDB/Redis integration 27 passed (2026-09-24). Two live Gemini 2.5 Flash-Lite calls returned 2,806 input tokens each, then 2,038 cache-read tokens and an estimated $0.00009798 on the hit (not a provider invoice). No OpenAI/Anthropic live inference credentials were available | Provider `usage` supplies observed total input/output and cache read/creation counters; missing streaming usage falls back to local token counting without claiming a hit. OpenAI and Gemini 2.5+ use provider implicit caching; direct Anthropic Chat Completions attaches a 5-minute breakpoint to a stable system block only for supported models, preserves explicit client breakpoints and skips custom bases/subscriptions. Anthropic `/v1/messages` keeps caller cache markers. Admin-set per-model cache prices take priority; missing direct-provider cache categories use an exact-ID LiteLLM bundled rate, never a guessed family alias, while custom bases require manual prices. Missing cache rates remain 0 USD and `partial`; LiteLLM-derived amounts are estimates, not provider-reported charges. Resolved rates and provenance freeze into durable run/summary/advisor snapshots, preserving old admitted prices. `chat_usage_logs` records uncached/read/creation-5m/creation-1h/output categories; owner `/v1/usage/records` exposes counters and credited cost, admin user detail exposes execution-model categorized cache USD and provenance (excluding advisor managed-component cost), `/v1/chat/completions` adds `usage.prompt_tokens_details.cached_tokens`, native `usage.updated` already carries cache components. Historical rows remain unsplittable; backend tier/surcharge differences need provider organization report reconciliation. Strict Afterglow SSE consumers must accept the previously introduced cache usage kinds before deployment. | `lumen/services/{usage_breakdown,litellm_client,completion_api,credit,chat_admission,stats,usage}.py`, `lumen/services/providers/{pricing,routing}.py`, `lumen/services/durable_runs/execution.py`, `lumen/api/compat/openai.py`, `lumen/migrations/014_prompt_cache_pricing.sql`, `docs/api-reference.md` |
...
| Kolla/Compose deployment | partial (cloud rollout unverified) | 7 workspace wheels built, SDK 125 passed; linux/amd64 and linux/arm64 API/worker/controller/test/sandbox images built and executed; 9 Compose system tests passed | 로컬 Compose API/worker/MariaDB/Redis/PostgreSQL은 검증되었다. Controller/Nova/Octavia/KVM guest의 실환경 배포와 접근통제는 아직 별도 검증이 필요하다 | `pyproject.toml`, `docker/Dockerfile`, `docker-compose*.yml`, `deploy/kolla/ansible/roles/lumen/`, `docs/operations.md` |
| Plugin wheel / registry / owner binding | implemented in source | seven workspace wheels built; five independent plugin conformance suites passed (88 passed, 1 skipped, 2026-09-24); isolated wheel-only registry installation exercised | `lumen_plugin_api`와 다섯 default 구현은 entry point와 operator allowlist로 선택된다. Frozen binding/config와 worker plugin digest는 MariaDB run/registration에 묶여 새 claim 시 재검증한다. 외부 plugin/provider live 서비스는 관찰하지 않았다 | `packages/lumen-plugin-api/`, `plugins/`, `lumen/plugins/`, migrations 015 |
| Resource intent, autoscaling controller, Nova | implemented in source; cloud acceptance unverified | 40 real MariaDB/Redis integration tests passed including resource operation fencing and active worker draining (2026-09-24); focused measured-load/controller tests passed | MariaDB intent/fence가 정본이며 worker generation/certificate는 migration 018에 고정된다. API 단일 guest process의 active request/SSE·60초 streaming text p95는 인증된 readiness proxy로 controller에 전달한다. Zun adapter source는 있으나 모든 Zun pool은 isolation 미강제로 설정 단계에서 거부한다. Nova/Octavia 실환경 수용은 입증하지 않았다 | `lumen/services/infrastructure/`, `lumen/main.py`, `lumen/controller.py`, migrations 016·018 |
| Per-run sandbox and durable child runs | implemented in source; cloud acceptance unverified | 40 real MariaDB/Redis integration tests passed including child budget/cancel/replay, repeated code-root slot/credit reuse across terminal paths, and claim/lease-takeover races committed between an unlocked read and the row lock under both `innodb_snapshot_isolation` modes (2026-09-24); linux/arm64 disposable Docker 21 sandbox tests passed with actual cgroup/tmpfs/netns/network/oom (re-run 2026-09-24); local arm64 daemon HTTPS bootstrap, mTLS readiness/no-cert denial and HMAC Python execution observed; linux/amd64 image built and 14 non-workload contracts passed | Effective `cgroup.subtree_control` delegation and exact one-rule default-drop nft policy required before workload/serving. amd64 real namespace tests require native runner (local emulation `bwrap` fails ENOSYS). v2 PostgreSQL checkpointer, project quota, explicit root budget, Nova pool and independent TLS required. Capabilities remain valid up to 15 seconds after lease revoke. Lock helpers refresh identity-mapped rows and the terminal barriers retry MariaDB 1020/1205/1213, but credit-segment start/settle transactions (`_credit_lineage`) are not retried and can surface 1020 under snapshot isolation when a sibling commits quota/root changes between their first read and lock. No Nova guest/KVM isolation, real controller/provider/Keystone or cloud child E2E was observed. | `packages/lumen-sandbox/`, `lumen/services/durable_runs/{children,budgets}.py`, `lumen/services/sandbox_binding.py` |

위 표의 `test-defined`는 해당 경계를 검사하는 테스트 코드와 명령이 정의되어 있다는 뜻이다. 2026-09-24 `uv run lumen-test contract -q`는 service 1283건을 통과하고 49건을 deselect했으며 root/SDK Ruff와 SDK 125건도 통과했다. `uv run lumen-test integration -q`는 일회용 실제 MariaDB/Redis에서 40건을 통과했다(active-path revision/branch, Gateway one-time expiring key lifecycle, child budget/cancel/replay, code-root slot/credit terminal release, 두 snapshot-isolation mode의 unlocked read→row lock claim/lease 경합 포함). `uv run lumen-test system -q`는 9건 모두 통과했으며 실제 API/worker/MariaDB/Redis process stack에서 fake provider를 상대로 Responses non-stream/SSE/function-call full-input continuation과 Gateway device authorization 및 Anthropic non-stream/SSE를 검증했다. 2026-09-20 별도의 isolated stack에서는 설치된 Codex CLI 0.154.0의 text turn과 실제 `exec_command` 후속 요청도 통과했다. 이는 외부 provider 호출, 실제 Keystone approval 또는 운영 배포 증거가 아니다.

최신 source 계약: direct Gemini/Anthropic exact LiteLLM catalog의 cache-read/5분 write/1시간 write `above_200k_tokens` 단가가 있으면 **단일 호출**의 provider-reported 총 prompt token이 200,000을 초과할 때만 해당 tier를 쓴다. 기본·고문맥 단가와 출처는 run/summary/advisor admission snapshot에 고정되며 수동 cache 가격이 우선한다. Advisor는 각 호출의 prompt 크기를 usage component에 동봉해 개별 tier로 정산한다. Frozen title job은 누락 입력/출력 가격에 public catalog를 적용하지 않는다. OpenAI virtual `lumen`의 cached tokens는 executor cache-read만 투영한다. Direct Anthropic의 자동 system breakpoint는 message뿐 아니라 명시적인 tool/request `cache_control`이 있는 경우에도 생략한다.

한 provider 응답에 여러 native compaction iteration이 합산되고 iteration별 TTL split이 없으면 cache tier 판정도 그 합산 입력에 적용된다. 내부 호출별 가격과 동일하다는 증거가 아니며 organization invoice 대조가 필요하다.

플러그인 경계: `default-skills`는 DB skill ID가 선택된 경우에만 필요하며 다른 승인된 skills binding만 고른 요청은 해당 provider에서 resolve한다. Per-user custom HTTP는 기존 `/v1/custom-tools`의 선택/인가 경로를 유지하고 plugin catalogue export를 추가하지 않는다. Remote MCP snapshot은 provider-safe한 공개 tool 이름과 별도의 원래 RPC method를 묶어 dispatch에 재사용한다. 이는 기존 authority·MariaDB journal 및 API/worker 책임 분리를 바꾸지 않는다.

Ingress enable/create는 pool lease row lock을 외부 SDK write가 완료될 때까지 잡아 stale controller의 재활성화를 직렬화한다. 다만 Octavia가 변경을 반영한 뒤 SDK가 실패하면 member ID가 원장에 없을 수 있으므로 operator는 `lumen-<resource_id>` 이름으로 남은 member를 확인·disable/delete해야 한다. Nova/Octavia live rollout로 승격하지 않는다.
...
flowchart LR
    ...
    Execute --> Journal
    Admission --> PluginHost[approved registry / frozen plugin bindings]
    PluginHost --> Wheels[database, memory, tools, skills, MCP wheels]
    Execute --> Child[child wait group and quota ledger]
    Child --> Journal
    Journal --> Intent[resource intent and worker registrations]
    Intent --> Controller[fenced lumen-controller]
    Controller --> Cloud[Nova VM / Zun adapter rejected pending isolation]
    Cloud --> Sandbox[per-run isolated sandbox]
    Worker --> Dispatch[controller-issued fenced capability]
    Dispatch --> Sandbox
    Sandbox --> Execute
...
| `lumen/worker.py`, `lumen/services/run_store.py` | DB/Redis claim, bounded tasks, bootstrap certificate-bound worker registration/heartbeat, plugin/protocol/ready-resource claim fence, SIGTERM drain | worker → durable runs/checkpointer/memory/resource store |
...
| `lumen/services/model_discovery.py`, `lumen/api/models.py` | Anthropic/Gemini native 및 OpenAI-compatible read-only candidate discovery; typed provenance·safe error·bounded HTTP | admin route → 저장 credential → provider; 모델 저장·가격·기능 추정 없음 |
...
| `lumen/services/tool_runtime/` | registry-backed builtin/custom HTTP/MCP/plugin binding, selection/schema/approval/dispatch | admission snapshot → worker-time plugin revalidation |
...
| `lumen/models/chat_db.py`, `lumen/models/chat_runs.py`, `lumen/models/chat_infrastructure.py` | catalog, run/event/lease, plugin binding, quota/child/resource intent/worker registration ORM | services → authoritative MariaDB |
...
| `packages/lumen-plugin-api/`, `plugins/`, `lumen/plugins/` | versioned Protocol/DTO·conformance, five independently packaged defaults, approved registry/host capabilities | core → public plugin API/registry; plugin wheels → public API only |
| `lumen/services/durable_runs/{children,budgets}.py`, `lumen/services/sandbox_binding.py` | durable child wait/join/settlement와 per-run fenced sandbox tool | graph/worker → MariaDB ledger/resource assignment → internal capability |
...
Reasoning effort는 admission에서 모델 capability로 검증해 run snapshot에 고정한다(`chat_admission._validated_reasoning_effort`). 명시 `none`은 omission으로 바꾸지 않고, models.dev capability가 끄기를 광고할 때만 허용한다. 광고 방식은 effort 목록의 `none`, `toggle`, `budget_tokens.min == 0` 중 하나이며, thinking이 opt-in인 `anthropic` provider도 허용한다. 그렇지 않으면 422다. 판정은 `capabilities.reasoning_can_be_disabled` 하나이며 `GET /v1/chat/models`가 같은 값을 `reasoning_none_supported`로 노출해 client가 규칙을 복제하지 않게 한다. LiteLLM 1.93은 `none`을 OpenAI gpt-5/o3에 그대로 전달하고 Gemini에는 `thinkingBudget: 0`으로 바꾸기 때문이다. 이를 위해 `models_dev._model_capabilities`는 `budget_tokens`의 정수 `min`/`max`를 저장하고, LiteLLM probe capability(`reasoning_options=[]`)는 끄기 미확인으로 본다. GPT-5 도구 호환성 검사는 이 판정 뒤에 실행되므로 `none`을 계속 허용하고, 거절 안내에는 모델이 지원할 때만 "없음"을 넣는다. 이 규칙은 unit/API test(`tests/test_chat_completions.py`, `tests/test_chat_models_dev.py`, `tests/test_chat_conversations.py`, `tests/test_ai_compat.py`)로만 확인했고 live provider에서는 확인하지 않았다. Title job은 별도 규칙(광고된 effort 목록의 가장 싼 값 또는 omission, 아래 title 문단)을 따른다. 요청 의도가 없는 운영자 기본값 `chat_reasoning_effort="none"`(stateless `completion_api` streaming 경로)은 같은 판정으로 끌 수 없는 모델에서 effort를 생략한다(omission). `graph.py`가 GPT-5 도구 요청에 붙이는 암묵적 `none`은 admission이 아니라 worker가 같은 판정을 frozen route에 적용해 결정한다(다음 문단).

GPT-5 도구 요청의 암묵적 `reasoning_effort="none"`은 모델 이름이 아니라 run의 frozen capability로 결정한다. Worker는 `config_version_hash`로 고정된 route(`ps.resolve_model_snapshot`)에서 `capabilities.reasoning_can_be_disabled`를 계산해 `durable_runs.execution` → `engine.stream` → `graph.stream`의 `reasoning_can_be_disabled`로 넘긴다(기본값 `False`). Graph는 OpenAI provider, `gpt-5*` 이름, effort `auto`/미지정, tool schema가 있는 라운드이면서 이 값이 참일 때만 `none`을 먼저 보낸다. models.dev가 minimal..high만 광고하는 gpt-5/gpt-5-mini는 provider 기본값으로 호출된다. 이전에는 이 모델들의 모든 도구 라운드가 거부된 `none`을 보냈고, durable hooks의 `provider_failed`는 항상 `provider_result_unknown` abort를 반환하므로 같은 라운드 재시도 없이 run이 실패했다. 잘못된 추측은 실패 시점, 즉 `provider_failed` boundary 전에 process-local로 기록한다. `none` 값 자체의 거부(`reasoning_effort`와 `none`을 담은 오류)는 `_TOOL_REASONING_NONE_REJECTED`에, 기본값 요청의 tool+reasoning 충돌은 `_TOOL_REASONING_EXPLICIT_NONE`에 넣는다. 따라서 durable run에서는 worker process·모델당 실패한 run이 최대 하나(기록 전 이미 진행 중이던 라운드 제외)이고 이후 run은 반대 형태로 호출한다. 같은 라운드 안의 재시도는 hooks가 abort하지 않는 경로에서만 동작한다. 그 밖의 오류(연결 끊김 등)는 기록하지 않는다. 암묵적 `none` 실패는 generic 재시도 분기로 넘기지 않으므로 `_REASONING_UNSUPPORTED`에 들어가 이후 명시 effort를 지우지 않는다. 기록은 기존 set처럼 모델 이름을 key로 하며 process 재시작 시 사라진다. Child run도 같은 worker 경로를 쓴다. 새 payload 필드·migration·저장소는 없다. 상태는 unit test(`tests/test_chat_graph.py`, `tests/test_chat_run_protocol.py`)로만 확인했다. Provider 오류 문구는 live provider에서 확인하지 않았다.

### Plugin selection, managed resources and child joins

`lumen-plugin-api`는 실행 DTO/Protocol만 게시한다. 관리자 allowlist의 distribution/name/version이 실제 entry point와 일치해야 registry가 시작되고, host는 필요한 capability만 플러그인에 전달한다. Native 요청은 DB HTTP tool/skill/MCP와 별도의 UUID plugin binding을 owner-scope로 resolve하여 immutable identity/schema/config digest를 freeze한다. Worker는 실행·approval 경계에서 재인가하며 버전/권한 변경을 fail-closed한다. 다섯 wheel은 core ORM/private import 없이 독립 설치되는 신뢰된 서버 코드이며 사용자 코드 sandbox와 다르다.

`plan`/`code`와 child delegation은 persistent v2 run, PostgreSQL checkpointer, 명시적 root `agent_budget` 및 operator project quota를 요구한다. Code root와 모든 child는 MariaDB sandbox intent와 함께 `waiting_resource`로 시작하고, controller가 cloud observation·bootstrap 인증서·sandbox readiness를 확인한 뒤 `queued`로 옮긴다. API는 선택한 plugin wheel set digest를 run에 고정하고 worker는 새 claim에서 registration boot/resource generation·fingerprint, protocol, digest 및 readiness를 검증한다. Worker는 `owner#lease_fence` 토큰으로 claim하며 controller는 live lease·worker certificate·resource generation을 확인해 15초 이내의 per-call capability를 발급한다. Guest HTTPS는 같은 TLS socket에서 peer pin/SAN 확인 뒤 source/bearer를 보낸다. 이미 발급된 capability는 DB lease revoke 후 만료 전까지 사용 가능한 창이 남는다. `run_code`는 별도 internal mTLS 경로만 사용하고 artifact를 worker가 pull/ingest하며 public SSRF policy는 유지한다.

`delegate_agent`는 승인된 같은 owner agent, frozen parent authority와 budget의 부분집합만 고른다. 한 model response의 child calls는 한 MariaDB wait group/ordinal로 저장되고 parent는 checkpoint-compatible children interrupt 후 `waiting_children`로 내려간다. Child completion은 결과·reservation 정산·join marker를 기록하고 DB scan/Redis wakeup으로 parent를 재개한다. Replay는 기존 group/call identity를 재사용하며 cancel은 descendant를 terminal cleanup으로 보낸다. Nova create의 lost response는 unknown intent로 유지해 중복 생성·임의 삭제를 금지한다. Zun은 adapter 코드가 있지만 sandbox host isolation 강제가 없어서 pool config가 모든 Zun 역할을 거부한다. 외부 OpenStack 제공자의 안전성/격리는 cloud 검증 전까지 입증되지 않는다.
...
`lumen/worker.py`는 capacity만큼 active task를 유지하면서 빈 slot을 즉시 채우고 독립 maintenance task로 stale run recovery, pending input/approval expiry, temporary thread purge, title/memory/background workspace jobs를 처리한다. Managed worker 등록은 bootstrap-issued generation/certificate fingerprint와 plugin digest를 DB에 고정한다. 새 claim은 현재 resource readiness·protocol·plugin digest·20초 이내 heartbeat·accepting을 검증하고, SIGTERM drain은 새 claim을 막지만 유효한 기존 lease의 capability를 허용한다. API guest의 독립 mTLS `/v1/ready`는 public process의 DB/plugin/checkpointer와 loopback-only active/SSE count·streaming 첫 text p95를 보고한다. Controller는 probe 실패 시 ingress를 disable하고 freshness 없는 telemetry로 scale-down하지 않는다. Redis wakeup은 최적화이고 MariaDB queued state·lease·journal이 복구 기준이다.
...
The title call (`lumen/services/title_summary.py`) sends a reasoning effort only when the frozen route's `capabilities.reasoning_options` advertises an `effort` list. It picks the cheapest listed value in the order `none → minimal → low → medium → high → xhigh → max`, so gpt-5 gets `minimal`, o3 gets `low`, gpt-5.1 gets `none`, and claude-opus-5-5 gets `low`. Routes with only `budget_tokens`/`toggle` options, empty options (LiteLLM/ChatGPT fallback capabilities), `reasoning=false`, or no capabilities omit the effort and keep the provider default. Examples are gemini-2.5-pro, where LiteLLM would map `none` to an invalid `thinkingBudget: 0`, and claude-sonnet-4-5. `litellm_client._reasoning_params` still drops a chosen effort when the provider probe reports no reasoning. Status: implemented, source-reviewed, test-passed. The evidence is focused unit tests, models.dev metadata (2026-09-26), and a LiteLLM 1.93 `get_optional_params` mapping probe; this is not live-verified against any provider. The change adds no service, store, or ownership boundary.
...
- **Agent-platform migrations 015/016**: `chat_plugin_bindings`는 owner/global export·암호화 config·version의 정본이고 `chat_runtime_*`, `chat_worker_registrations`, `chat_agent_reservations`, `chat_delegation_*`는 quota/resource generation/child join의 정본이다. `chat_runs.lease_fence`는 stale worker를 차단한다. Controller와 worker는 같은 MariaDB schema/image/plugin 버전을 사용해야 하며 migration checksum은 manifest와 일치해야 한다.
...
- **Internal sandbox boundary**: sandbox의 per-call SQLite journal은 같은 resource의 idempotent local result만 보존하고 LangGraph checkpoint/Redis와 마찬가지로 Lumen의 run/event 정본을 대체하지 않는다. Operator cloud credential과 dispatch key는 controller 전용; worker에는 발급된 client identity만, sandbox에는 해당 resource/run/generation의 파생 key만 들어간다. Public custom HTTP/MCP SSRF restriction은 내부 private transport에 적용되지 않으며 서로 공유하지 않는다.
...
관리자 discovery는 Anthropic `x-api-key`/`anthropic-version` + `after_id`, Gemini `x-goog-api-key` + `pageToken`, OpenAI-compatible Bearer `/models`로 계정별 후보를 조회한다. 기존 Perplexity router·custom base와 subscription namespace를 보존한다. HTTP client는 redirects/environment proxy를 사용하지 않고 전체 20초·25페이지·2,000 unique 모델·5MB streamed identity response로 제한한다. 정상 empty는 complete API 결과다. 오류나 중간 페이지 실패·한도 초과는 모델 목록 전체를 폐기하고 safe error로 반환하며 static fallback을 하지 않는다. Static 참고 목록은 live 미지원 provider/auth에만 제공한다.

`available-models`는 no-store와 `provider_id`, `fetched_at`, `source`, `live_status`, `complete`, `error`, `models`, `candidates`를 반환한다. Upstream 401/403은 caller session 오류와 구별되는 HTTP 200 structured discovery error다. 후보의 exact opaque ID·표시명·generation methods·token limits는 등록/가격/고급 기능/실제 추론 증거가 아니다. 신규 ID는 기존 CRUD와 request-time DB route를 사용하며 native admission은 가격 쌍과 별도 feature gate를 검사한다. LiteLLM 계산기가 unknown ID에 0을 반환해도 exact catalog entry의 명시 단가 없이는 free 가격으로 인정하지 않는다. 명시적 0 USD와 미확인 `null`을 구분한다.

## Deployment and operations

로컬 Compose(`docker-compose.yml`)는 MariaDB, Redis, migration/backfill, idempotent `seed-local`, API, worker, opt-in connection helper와 localhost Console을 분리한다. API 기본은 `127.0.0.1:8012`다. Provider key 없이 stack은 기동해도 completion은 실행되지 않는다. `lumen-migrate --apply`가 migration 012 projection backfill/integrity, 013 Gateway schema와 014 prompt-cache price/ledger column을 완료한 뒤 API/worker가 시작된다. Gateway local base는 loopback HTTP를 허용하지만 model/provider route가 없으면 inference는 503이다.
...
Service 이미지는 root `service` extra의 workspace plugin distributions와 editable source 경로를 함께 보존한다. `docker/Dockerfile`은 workspace manifests를 먼저 복사하고 `uv sync --locked`로 manifest/lock 불일치를 build 단계에서 거부한다. 최종 API/worker/controller 공통 runtime은 `packages/lumen-plugin-api`와 `plugins/`를 포함하고 non-root 사용자로 실제 `python -m lumen.scripts.migrate --help`를 실행해 import 누락을 검출한다. 이 검사는 DB에 연결하거나 migration을 적용하지 않는다. Sandbox는 이 trusted runtime과 별도 stage이며 service workspace에 포함하지 않는다.
...
- **Kolla managed-runtime identity**: controller만 root-owned 서명 키·CA private key를 읽기 위해 컨테이너 root로 기동하며 Docker socket을 받지 않는다. Stock worker에는 별도 제한된 디렉터리의 public CA·client cert/key 세 파일만 group-readable read-only로 마운트하고, 동일한 container TLS 경로에서 operator key나 CA private key를 노출하지 않는다. Precheck가 파일 위치·소유권·모드·허용된 세 파일만 검증하며 Kolla TOML의 runtime JSON은 문자열로 escape한 뒤 설정 loader가 typed object로 복원한다. Source mode는 controller target도 빌드한다. Compose의 수동 controller profile은 격리/cloud acceptance가 아니다.
...
| plugin authoring/selection | `packages/lumen-plugin-api/`, `plugins/`, `lumen/plugins/`, migrations 015 | independent wheel conformance와 operator allowlist, owner-scoped binding snapshot/revoke, plugin status/API/SDK, `docs/agent-platform.md` |
| child/agent quota and checkpoint | `lumen/services/durable_runs/{children,budgets,execution}.py`, `lumen/services/graph.py`, migrations 016 | MariaDB lock order·idempotent join/cancel/settlement, v2 encrypted checkpoint, owner child API, `docs/agent-platform.md` |
| Nova/Zun/runtime identity | `lumen/services/infrastructure/`, `lumen/controller.py`, `packages/lumen-sandbox/`, `docker/Dockerfile` | operator cloud project separation, bootstrap/TLS, real isolation and multiarch image, `docs/operations.md`/`docs/security.md`; mock != live |
...
{
  "schema_version": 1,
  "source_sha256": "5bfeee1207d379c57b74dad237d20e12c30c91644af3a893814020d6f23d33d1",
  "reviewed_at": "2026-09-26T14:26:05Z",
  "summary": "Rebased onto origin/dev (bbb25d1 title effort, e1cbcc1 admission none gate, c64ea88 operator none omission). Reviewed staged lumen/services/{graph,engine}.py, durable_runs/execution.py and tests/test_chat_{graph,run_protocol}.py: GPT-5 tool-round implicit reasoning none now requires frozen capabilities.reasoning_can_be_disabled computed by the worker and passed through engine.stream to graph.stream (default False); none rejections and tool+reasoning conflicts are memoized process-locally before the always-aborting provider_failed boundary. Merged the admission paragraph pointer. No new module, payload field, storage, migration or route; unit-tested only, not live-verified."
}
```
