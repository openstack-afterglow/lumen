...
| Provider-native server-side compaction | implemented | focused compaction tests passed (59 passed, 2026-09-22); synthetic provider payloads, no live Anthropic compaction observed | Anthropic-only on the chat path because the pinned LiteLLM transport forwards `context_management` there and nowhere else. Trigger is absolute, floored at 50,000 tokens, and left unarmed when the window cannot express the ratio. Compaction blocks round-trip within a run. The OpenAI-compatible and native durable surfaces share the same fence; the stateless passthrough protocols do not own conversation state and are exempt. | `lumen/services/native_compaction.py`, `lumen/services/graph.py`, `lumen/services/durable_runs/execution.py` |
...
Provider-native compaction layers under, not over, that fence. `native_compaction.py` resolves an Anthropic `context_management` edit at the same 0.80 occupancy ratio, but the provider trigger is an absolute input-token count against the raw window while the Lumen fence divides by `input_budget`, so the Lumen fence always trips first and stays the durable, provider-neutral mechanism. The edit is armed only where the pinned LiteLLM chat transport forwards `context_management` — Anthropic today; every other route resolves to `None` rather than sending a parameter `drop_params` would silently discard. A window that cannot express the ratio above the documented 50,000-token floor is left unarmed rather than clamped to a trigger that can never fire. `chat_native_compaction_enabled` disables it without touching Lumen's own compaction. Returned `compaction` blocks are carried on every later assistant turn of the run, including replayed and checkpoint-resumed turns, because a dropped block makes the provider re-compact and re-bill the same prefix; blocks are bounded and re-validated before they are sent back. Compaction blocks are carried within a run, not across conversation turns — cross-turn continuity remains Lumen's encrypted summary checkpoint. This adds no service, database schema or ownership boundary.

The automatic fence is a property of the durable run, not of the browser. `create_temp_run` materializes a temporary thread even when the caller supplies none, so the OpenAI-compatible `/v1/chat/completions` surface produces a parented run and is fenced exactly like the native conversation endpoints, with the same resolved summary route. Only a run with neither a conversation nor a temporary thread is exempt, and no current surface produces that shape. The stateless passthrough protocols reach the provider without a durable run at all and are therefore outside the fence by construction.
...
- **Authentication I/O scheduling**: async `get_principal`/`require_token`은 동기 Keystone `validate_token`을 bounded Starlette thread pool에서 기다린다. 동일한 token rescope, target-project/admin 판정과 실패 응답을 유지하며 auth cache나 fail-open을 추가하지 않는다. 느린 Keystone 검증 중에도 다른 HTTP 요청과 health가 진행되는 ordering regression으로 검사한다.
...
- **Secrets**: `LUMEN_ENCRYPTION_KEY`는 64 hex이며 AES-GCM/HKDF domain separation으로 chat content, inference provider key, billing administrator key, extension content를 분리한다. Billing administrator key는 direct OpenAI/Anthropic organization report에만 전달하고 inference/custom base에는 사용하지 않는다. API key는 hash만 MariaDB에 저장하고 발급 응답에서만 plaintext를 노출한다. provider, MCP, Git, S3 secret과 raw tool args는 logs/journal/response에 기록하지 않는다.
