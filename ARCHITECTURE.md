...
| Prompt-cache pricing and Anthropic-shaped usage ledger | implemented | focused tests passed (`tests/test_chat_cache_pricing.py` 45 passed; full unit suite `pytest tests --ignore=tests/integration --ignore=tests/system` 1204 passed, 1 skipped, 2026-09-23); synthetic provider payloads plus LiteLLM's real `AnthropicConfig.calculate_usage`, no live provider cache observed | Cache rates live on the `llm_models` price record (`cache_read_price`, `cache_write_price` for the 5-minute TTL, `cache_write_1h_price`; admin API per-million, optional and independent). One `UsageBreakdown` (`lumen/services/usage_breakdown.py`) converts LiteLLM `Usage`, raw Anthropic Messages usage (input excludes cache; iterations summed including cache) and raw Responses usage (input includes cached, never added twice) into total input, output, cache read, cache creation 5m/1h; creation without a TTL split is 5m, a reported 1h share is clamped to the creation total. `chat_usage_logs.prompt_tokens` stays TOTAL input and the three new token columns hold the split; uncached input is derived. Cost = uncached×input + read×cache_read + 5m×cache_write + 1h×cache_write_1h + output×output in both `cost_from_usage` and the frozen-snapshot path; `raw_cost` equals the sum of component costs. Every path (durable executor aggregate and summary route, `/v1/chat/completions`, `/v1/responses`, `/v1/messages` non-stream and stream with `message_start`+cumulative `message_delta` merge, title, memory) uses the breakdown, and executor `usage_components` carry `input_tokens` as the uncached quantity plus cache kinds when non-zero. Cache rates join `config_version_hash` only when at least one is set, so existing hashes stay byte-identical. RESIDUALS: (1) an unset cache rate bills that category at 0 USD and marks the usage `partial` — there is deliberately no LiteLLM/models.dev fallback, so deploying this lowers the cost of cached traffic on both LiteLLM-normalized paths — durable web/native chat runs and `/v1/chat/completions` — because cache reads/creation are no longer billed at the input rate until an administrator sets rates, and `stats` counts those rows as partial; (2) runs admitted before this change carry snapshots without cache keys and bill cache at 0 even after a rate is set; (3) legacy ledger rows cannot be split: old chat rows have cache folded into `prompt_tokens` and priced at the input rate, old `/v1/messages` rows under-count cache tokens (raw `input_tokens` excluded cache) — the new columns are 0 for all of them; (4) the LiteLLM token-counter fallback carries no cache split (a `/v1/messages` stream cut before `message_stop` keeps the `message_start` input/cache split and only falls back for output, unless that report has no input at all — LiteLLM's non-Anthropic `/v1/messages` adapters open with an all-zero usage, so the prompt is then counted locally without a cache split); the managed advisor bills uncached input at the advisor input rate and its cache categories from the advisor model's manual cache rates frozen into `component_prices` (missing → 0 USD, component `metadata.unpriced`, run `partial`), and other managed tools carry no token cache; (5) new `/v1/messages` rows record larger `prompt_tokens` than before because cache read/creation is now included in total input, so ledger/stats token totals jump for that traffic while its cost is unchanged until rates are set; (6) the durable chat path sees LiteLLM's streamed `chunk.usage`; `calculate_usage` and the Anthropic chunk parser keep the split (probed), but the stream wrapper's final merged usage was not probed end to end; (7) the new usage kinds (including `advisor_cache_*`) require consumers that validate `usage.updated` kinds strictly (the Afterglow frontend `chatContracts.ts`) to accept them before this ships: deploy the Afterglow frontend that accepts them first, then this Lumen; the reverse order stalls every web run with cache tokens at `usage.updated`. | `lumen/services/usage_breakdown.py`, `lumen/services/litellm_client.py`, `lumen/services/credit.py`, `lumen/services/completion_api.py`, `lumen/services/durable_runs/execution.py`, `lumen/services/providers/{pricing,routing,repository,billing}.py`, `lumen/migrations/014_prompt_cache_pricing.sql` |
