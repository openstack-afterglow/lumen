...
- **휠 패키지 빌드**: `deploy/kolla`에서 Hatchling 빌드를 통해 `lumen_kolla-<release-version>-py3-none-any.whl` 아티팩트가 생성된다.
- **Kolla 환경 설치**: Kolla Ansible 가상환경(`python 3.11`)에 `pip install lumen_kolla-<release-version>-py3-none-any.whl`을 수행하면 역할 자산이 `share/kolla-ansible/ansible/roles/lumen`에 설치된다.
...
- **락스텝 버전 관리**: `lumen.__version__`, Kolla 역할 default `lumen_image_tag`, `lumen-kolla` 패키지 버전은 엄격히 동기화된다.
- **기본 이미지 네임스페이스 및 태그**: Lumen 역할 기본값은 `ghcr.io/openstack-afterglow/lumen-api:<release-version>` 및 `ghcr.io/openstack-afterglow/lumen-worker:<release-version>`을 사용하며 Afterglow release tag에 종속되지 않는다. Operator는 exact digest ref override를 그대로 유지할 수 있다.
