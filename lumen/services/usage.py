...
async def user_usage_summary(user_id: str, project_id: str = "", api_key_id: int | None = None) -> dict:
    """본인 누적/이번달·이번주 사용량과 기간별 쿼터. 실패 시 빈 요약(found=False).
    ...
    try:
        ...
        async with factory() as session:
            ...
            week_credited = (
                await session.execute(
                    select(_SUMS[0]).where(
                        *base,
                        ChatUsageLog.created_at >= week_start(),
                    )
                )
            ).scalar_one()
            src_rows = (
        ...
        return {
            ...
            "week_credited_cost": float(week_credited or 0),
